...
export default function Planner() {
  ...
  async function load() {
    ...
    if (results.some((result) => result.status === "rejected")) setToast("Dalies duomenų atnaujinti nepavyko. Išsaugoti duomenys tebėra rodomi.");
    setLoading(false);
  }
  ...
  async function saveEvent(event:CalEvent,patch:Record<string,unknown>) {
    ++loadVersion.current;
    try {
      const updated=await responseJson<CalEvent>(await fetch(`/api/${event.provider==="outlook"?"microsoft":"google"}/events`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({id:event.id,connectionId:event.connectionId,version:event.version,...patch})}));
      setEvents(current=>current.map(item=>item.id===event.id && item.provider===event.provider && item.connectionId===event.connectionId ? updated : item));
...
function TimeGrid({ days, events, tasks, onDrop, onCreate }: { days: Date[]; events: CalEvent[]; tasks: Task[]; onDrop: (e: DragEvent<HTMLDivElement>, d: Date) => void; onCreate: (d: Date) => void }) {
  ...
  const timed = [
    ...events.filter(event => !event.allDay && !tasks.some(task => task.mirror_event_id === event.id && event.provider === "outlook" && !task.completed && task.scheduled_at)).map(event => ({key:`event-${event.provider}-${event.connectionId}-${event.id}`,start:new Date(event.start.dateTime!),end:new Date(event.end.dateTime!),event,task:undefined})),
...
function TaskBlock({ task, segment }: { task: Task; segment:DaySegment }) {
  ...
  return <div className="eventBlock taskTime" data-short={segment.height<45 || undefined} data-tiny={segment.height<24 || undefined} style={{...segmentStyle(segment,preview),transform:offset ? `translate(${offset.x}px,${offset.y}px)` : undefined,zIndex:offset ? 10 : undefined,pointerEvents:offset ? "none" : undefined}}>
    <button className="taskBlockEdit" disabled={saving} aria-label={`Redaguoti planą: ${task.title}`} title={segment.gestureSafe ? "Tempk į kitą dieną arba paspausk redaguoti" : "Kelių dienų ar laiko keitimo dienos planą keisk paspaudęs redaguoti"}
